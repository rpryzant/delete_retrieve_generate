...
def decode_minibatch(max_len, start_id, model, src_input, srclens, srcmask,
        aux_input, auxlens, auxmask, diag_input_lines, diag_lens, diag_mask, coexist_attr):
    """ argmax decoding """
    ...
    for i in range(max_len):
        ...
        decoder_logit, word_probs = model(src_input, tgt_input, srcmask, srclens,
            aux_input, auxmask, auxlens, diag_input_lines, diag_lens, diag_mask, coexist_attr)
...
def decode_dataset(model, src, tgt, config):
    ...
    for j in range(0, len(src['data']), config['data']['batch_size']):
        ...
        input_content, input_aux, output = data.minibatch(
            ...
            is_test=True)
        input_lines_src, output_lines_src, srclens, srcmask, indices, diag_input_lines, diag_lens, diag_mask, coexist_attr = input_content
